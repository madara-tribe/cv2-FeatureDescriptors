...
def surf_descriptor(grayscale_img):
    surf = cv2.xfeatures2d.SURF_create()
    kp, des = surf.detectAndCompute(grayscale_img, None)
    return kp, des


def surf_homography(img1, img2):

    kp1, des1 = surf_descriptor(img1)
    kp2, des2 = surf_descriptor(img2)
    bf = cv2.BFMatcher()
